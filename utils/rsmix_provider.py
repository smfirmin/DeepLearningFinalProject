# Source: https://github.com/dogyoonlee/RSMix/blob/dff7785b2be8372f6ca557dd1372bebc7d27691f/pointnet2_rsmix/utils/rsmix_provider.py
...
# from tf_grouping import knn_point
...
def query_ball_point_for_rsmix(radius, nsample, xyz, new_xyz):
    ...
    # print("group idx init : \n",group_idx)
    # print("group idx init shape : ",group_idx.shape)
...
def rsmix(data_batch, label_batch, beta=1.0, n_sample=512):
    n_sample = int(np.around(data_batch.shape[1]/2))
    ...
    for i in range(data_batch.shape[0]):
        if pts_erase_idx[i][0][0]==data_batch.shape[1]:
            ...
            # if len(pts_add_idx_tmp) >= len(pts_add_idx_ctrled_tmp):
            #     lam_tmp = len(pts_add_idx_ctrled_tmp)/(len(pts_add_idx_ctrled_tmp)+len(tmp_pts_erased))
            # else:
            #     lam_tmp = len(pts_add_idx_tmp)/(len(pts_add_idx_tmp)+len(tmp_pts_erased))
            lam_tmp = len(pts_add_idx_ctrled_tmp)/(len(pts_add_idx_ctrled_tmp)+len(tmp_pts_erased))
...
def rsmix_for_part_seg(data_batch, beta=1.0, n_sample=1024):
    ...
    return data_batch_mixed

# def rsmix_knn(data_batch, label_batch, beta=1.0, n_sample=512):
#     n_sample = int(np.around(data_batch.shape[1]/2))
#     cut_rad = np.random.beta(beta, beta)
#     rand_index = np.random.choice(data_batch.shape[0],data_batch.shape[0], replace=False) # label dim : (16,) for model
...
#     query_point_1 = np.zeros((data_batch.shape[0],1,data_batch.shape[2]))
#     query_point_2 = np.zeros((data_batch.shape[0],1,data_batch.shape[2]))
#     # print("idx : \n",idx)
#     for i in range(data_batch.shape[0]):
#         query_point_1[i][0]=data_batch[i][rand_idx_1[i][0]] # Bx1x3(=6 with normal)
#         query_point_2[i][0]=data_batch[i][rand_idx_2[i][0]] # Bx1x3(=6 with normal)
        
#     k_para = np.randint(n_sample)
    
#     _, pts_erase_idx = knn_point(k_para, xyz1=data_batch, xyz2=query_point_1)
#     _, pts_add_idx = knn_point(k_para, xyz1=data_batch_rand, xyz2=query_point_2)
#     # pts_erase_idx, query_point_1 = cut_points(data_batch, rand_idx_1, cut_rad, nsample=n_sample) # B x num_points_in_radius_1 x 3(or 6)
#     # pts_add_idx, query_point_2 = cut_points(data_batch_rand, rand_idx_2, cut_rad, nsample=n_sample) # B x num_points_in_radius_2 x 3(or 6)
